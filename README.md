...
Current marketing version: `0.14.0` (build `9`), targeting backend app `0.14.0`, API contract `20260715_010` and finance schema `20260715_006_semantic_preference_memory_v2`. The delegated contract did not provide a frozen mainline commit, and the local mainline checkout was still v0.13.0 when this client snapshot was prepared; the validator therefore fails closed in a full workspace until mainline publishes the same matrix. The Profile screen reads `CFBundleShortVersionString` from the built app bundle, so the displayed version follows Xcode build settings rather than a hardcoded UI value.
...
The frozen preference-memory mapping and its safety requirements remain documented in `Docs/PERSONALIZATION_CONTRACT_REQUIREMENTS.md`. Historical v0.13.0 and earlier fixtures remain independent compatibility baselines.
