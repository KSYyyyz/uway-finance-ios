...
- Native login/registration switching, SMS challenge countdown and HttpOnly-cookie session restoration. Registration is enabled only when the server publishes the exact SMS-webhook, isolated-organization/account-book and secure-cookie capability; there is no client-generated code or fallback registration.
...
Registration secrets exist only in transient SwiftUI state and JSON request bodies. Passwords and one-time codes are never put in URLs, logs, `UserDefaults` or Keychain plaintext; the server owns challenge TTL, resend timing, rate limiting and delivery failure. The frozen preference-memory mapping and its safety requirements remain documented in `Docs/PERSONALIZATION_CONTRACT_REQUIREMENTS.md`. Historical v0.13.0 and earlier fixtures remain independent compatibility baselines.
