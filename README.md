...
- Registration requires separate phone and email challenges, applies the server-equivalent NFKC/lowercase username grammar and password identity checks, and performs a 450 ms debounced availability request. Email verification is enabled only for the exact `registration_verification` capability with `email_webhook` or `aliyun_direct_mail`; password-reset challenges are never reused. Availability is UX only; server validation and its unique indexes remain authoritative.
...
Authentication secrets exist only in transient SwiftUI state and JSON request bodies. Phone registration codes, email registration codes, password-reset codes and their challenge IDs are separate transient values and are never put in URLs, logs, `UserDefaults` or Keychain plaintext. The server owns purpose isolation, digest storage, TTL, resend/attempt limits and delivery failure. The frozen preference-memory mapping and its safety requirements remain documented in `Docs/PERSONALIZATION_CONTRACT_REQUIREMENTS.md`. Historical v0.15.0 and earlier fixtures remain independent compatibility baselines.
