...
Current marketing version: `0.13.0`, compatible with backend app `0.13.0`, API contract `20260715_009`, finance schema `20260715_005_immutable_record_evidence` and frozen mainline commit `7bd702c`. The Profile screen reads `CFBundleShortVersionString` from the built app bundle, so the displayed version follows Xcode build settings rather than a hardcoded UI value.
...
- Immutable business-record evidence is capability-gated behind `BusinessRecordEvidenceAPI`. Each record detail resolves the selected server account book, shows attachment/invoice/payment coverage, lists active and revoked originals, uploads photos or files with multipart metadata before bytes, verifies viewed content against byte length/SHA-256/ETag, and marks evidence revoked without deleting content. Upload and revoke retries retain their file/form and reuse a key only for the same logical request; changing account book or record clears all evidence state.
...
The frozen preference-memory mapping and its safety requirements remain documented in `Docs/PERSONALIZATION_CONTRACT_REQUIREMENTS.md`. Historical v0.12.0 and earlier fixtures remain independent compatibility baselines.
