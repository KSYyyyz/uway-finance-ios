...
Current marketing version: `0.15.0` (build `14`), targeting backend app `0.15.0`, API contract `20260720_013` and finance schema `20260720_010_account_identity_recovery`. The native authentication flow now negotiates username/phone/email login, email-required registration, debounced username availability and enumeration-resistant email password recovery. The validator cross-checks the live workspace contract and fails closed on version, route, capability, tenant-isolation or recovery-safety drift. The Profile screen reads `CFBundleShortVersionString` from the built app bundle, so the displayed version follows Xcode build settings rather than hardcoded UI text.
...
- Registration requires email, applies the server-equivalent NFKC/lowercase username grammar and password identity checks, and performs a 450 ms debounced availability request. Availability is UX only; server validation and its unique index remain authoritative.
...
- Every password and confirmation field has an accessible show/hide action. Passwords, verification codes, identifiers, provider credentials and tokens remain transient and are never written to logs, URLs, preferences or plaintext credential storage.
...
It runs for iOS or checked-in contract-snapshot changes and can also be started manually with `workflow_dispatch`. `ContractSnapshots/backend-api-v0.15.0.json` is the standalone frontend contract baseline; when the repository is located inside the full Uway workspace, the validator additionally cross-checks the local backend routes, identity/password policy, recovery implementation, capability factory, evidence immutability rules and Finance Domain schema constant. No Apple signing secret is required for this simulator job.
...
Authentication secrets exist only in transient SwiftUI state and JSON request bodies. Passwords, one-time codes, reset challenges and complete identifiers are never put in URLs, logs, `UserDefaults` or Keychain plaintext; the server owns challenge TTL, resend timing, rate limiting and delivery failure. The frozen preference-memory mapping and its safety requirements remain documented in `Docs/PERSONALIZATION_CONTRACT_REQUIREMENTS.md`. Historical v0.14.1 and earlier fixtures remain independent compatibility baselines.
