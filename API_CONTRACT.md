...
The backend is deployed independently on Alibaba Cloud and is not part of this frontend repository. `ContractSnapshots/backend-api-v0.16.1.json` is the checked-in iOS 0.16.1 baseline used by Windows and macOS CI; it targets backend app 0.16.1, API contract `20260721_014` and schema `20260721_011_verified_account_email`. Full-workspace validation cross-checks actual root code and contract documents; all 0.16.0 and earlier snapshots/fixtures remain unchanged as compatibility baselines.
...
| POST | `/api/auth/registration-email/resend` | `FinanceAPI.resendRegistrationEmail(...)` |
| POST | `/api/auth/registration-email/confirm` | `FinanceAPI.confirmRegistrationEmail(...)` |
...
Registration is capability-gated. The unchanged 0.16.1 contract requires the phone-code and registration endpoints plus `usernameAvailabilityEndpoint`, `phoneVerification=aliyun_sms`, `emailRequired=true`, `emailVerificationRequired=true`, `usernameNormalization=nfkc_lowercase`, username length 3–32, password length 8–256 and the secure session-cookie boundary. The email sub-capability must publish `strategy=email_link`, exact confirm/resend endpoints, `tokenStorage=hmac_sha256_digest_only`, `tokenTransport=url_fragment_then_post_body`, `activation=after_email_confirmation`, `pendingTenantCreated=false` and an indistinguishable unknown-pending response. Missing or unknown values disable registration rather than falling back to the withdrawn email-code flow.

`POST /api/auth/register` sends `{username,email,password,phone,challengeId,code}` and receives HTTP 202 `{ok,pendingRegistrationId,expiresInSeconds,resendAfterSeconds,message}`. The response deliberately has no user, organization, account book or session. Resend posts only `{pendingRegistrationId}` and receives the same indistinguishable shape; a successful resend rotates the server token and invalidates the old link.

The email link carries `#verify-email=<token>`. Web clears the fragment before posting `{token}` to the confirm endpoint; a 201 response is the only registration response containing the activated user, organization and account-book IDs. iOS models and tests that JSON-body confirm boundary but does not claim Universal Link routing or browser-cookie sharing until those platform contracts are frozen. After Web confirmation, the native pending screen returns the user to ordinary identifier/password login. The registration password row has one accessible visibility control that also changes confirmation-field visibility.

Password recovery is shown only when the exact capability publishes both endpoints, email-webhook delivery, HMAC-digest-only code storage, indistinguishable unknown-email responses and all-session revocation. Request responses are displayed without inferring whether an email exists. Confirmation sends `{email,challengeId,code,newPassword}` and immediately rotates the local session generation, clearing legacy state, revisions, conflicts, account-book caches and temporary previews. `EMAIL_RESET_UNAVAILABLE`, request rate limits, expired/invalid codes and weak-password errors keep entered form state unless the server declares the challenge invalid.

The client recognizes the frozen SMS, pending-link and provider errors without erasing unrelated input: `INVALID_REGISTRATION_CODE`, `INVALID_REGISTRATION_EMAIL_TOKEN`, `REGISTRATION_EMAIL_RESEND_RATE_LIMITED`, `REGISTRATION_EMAIL_CONFIRM_RATE_LIMITED`, `REGISTRATION_EMAIL_LINK_RATE_LIMITED`, `REGISTRATION_IDENTITY_CONFLICT` and `EMAIL_VERIFICATION_UNAVAILABLE`. Server SMS TTL expiry clears only the local SMS challenge/code; link errors keep the pending ID so a permitted resend can recover.

Login, email-link confirmation and logout start a new session generation. Pending registration and resend remain signed out and never fetch `/api/state`. Responses may update UI/state only while their generation and expected user remain current, and live authentication requests are serialized so an older slow cookie response cannot land after a newer identity request. Account switch, logout or `401` clears `/api/state`, revision, unsaved/conflict snapshots, all account-book coverage/V2 view caches and temporary preview files before another identity can render. No cache key is shared across authenticated users even when an account-book ID string happens to match.

`PUT /api/state` remains the compatibility bridge and now provides whole-state optimistic concurrency. Capabilities advertise `conflictControl=optional_if_match`, `versionSource=updatedAt`, `etagHeader=ETag` and `conditionalWriteHeader=If-Match`. Older clients may temporarily omit the header, but iOS 0.16.1 never does.
